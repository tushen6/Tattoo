...
if (isQuark) {
  ...
  if (downloadUrl) {
   ...
   async function homeContent() {
     try {
       const url = `http://154.204.177.231`;
   ...
   async function searchContent(keyword) {
       try {
           ...
           const url = `http://154.204.177.231/index.php/vod/search.html?wd=${encodedKeyword}`;
   ...
   async function categoryContent(tid, pg = 1, extend) {
    ...
    let url = `http://154.204.177.231/index.php/vod/show/area/{area}/by/{by}/class/{cateId}/id/${tid}/page/${pg}/year/{year}.html`;
    // 替换 URL 中的占位符
    if (extendObj) {
...
async function detailContent(ids) {
  const url = `http://154.204.177.231${ids}`;
  try {
    //console.log(url);
