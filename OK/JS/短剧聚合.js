...
function safePushResult(target, seen, item, wdLower = '') {
    ...
    if (wdLower && !keywordMatched(item.vod_name, wdLower)) return;
    if (seen && seen.has(item.vod_id)) return;
    if (seen) seen.add(item.vod_id);
    target.push(item);
}
        ...
        searchPromises.push((async () => {
            ...
            try {
                ...
                if (tid === '星芽') {
                    ...
                    data = (res?.data?.theater?.search_data || []).map(i => ({ vod_id: `星芽@${plat.host}${plat.url2}?theater_parent_id=${i.id}`, vod_name: i.title, vod_pic: i.cover_url, vod_remarks: `星芽短剧｜${i.total}集 播放:${i.play_amount_str}` }));
                } else if (tid === '西饭') {
                    ...
                    data = (res?.result?.elements || []).map(vod => {
                    ...
                    res = await request(`${plat.host}${plat.search}?version_code=1500&version_name=1.5.0&device_name=${encodeURIComponent(deviceName)}&device_type=phone&is_first_day=true&is_first_24h=true&app_launch_way=icon&default_homepage=homepage_interaction&device_owning_firm=${encodeURIComponent(deviceFirm)}&font_scale=default&os_type=1&clientInfo=${clientInfo}`, {
                        method: 'POST',
                        headers: { 'User-Agent': 'okhttp/5.1.0', 'Content-Type': 'application/json; charset=utf-8' },
                        body: { audience: '', order: '', page: pg, pageSize: 30, searchWord: wd, subject: '' },
                        timeout: 10000
                    });
                    data = (res?.data || []).map(i => ({ vod_id: `围观@${i.oneId}`, vod_name: i.title, vod_pic: i.horzPoster || i.vertPoster, vod_remarks: `围观短剧｜${i.episodeCount || ''}集` }));
                } else if (tid === '河马') {
                    res = await request(`${plat.host}${plat.search}`, {
                        method: 'POST',
